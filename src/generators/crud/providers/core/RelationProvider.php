        ...
        if ($relation) {
            switch (true) {
                case !$relation->multiple:
                    ...
                    $method = __METHOD__;
                    $relationModelClass = $this->generator->modelClass;
                    $relationGiiModelClass = str_replace('\\models\\','\\gii\\',$relationModelClass);
                    $useSelect2 = (
                            class_exists($relationGiiModelClass)
                            && property_exists($relationGiiModelClass,'giiantUseSelect2')
                            && $relationGiiModelClass::$useSelect2
...
\$form->field(\$model, '{$column->name}')->dropDownList(
    \yii\helpers\ArrayHelper::map({$relation->modelClass}::find()->limit(1000)->all(), '{$pk}', '{$name}'),
    [
